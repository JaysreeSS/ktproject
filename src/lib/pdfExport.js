...
export const exportProjectToPDF = (project) => {
    ...
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(30);
    doc.text(project.name.toUpperCase(), margin, cursorY);
    cursorY += 8;
    ...
    project.sections.forEach((section, index) => {
        ...
        doc.text(`${index + 1}. ${section.title.toUpperCase()}`, margin, cursorY);
        cursorY += 6;
        ...
        doc.setTextColor(100);
        doc.text(`Assignee: ${assignee?.name || 'Unassigned'} (${assignee?.ktRole || '-'}) | Status: ${section.status}`, margin, cursorY);
        cursorY += 8;
    ...
    for (let i = 1; i <= totalPages; i++) {
        ...
        doc.text(`KT Project Management System - Confidental`, margin, doc.internal.pageSize.getHeight() - 10);
    }
